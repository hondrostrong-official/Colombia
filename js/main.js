...
$('a[href*="#"]').click(function() {
    ...
    Landing.prototype.loadCityes = function() {
        var _this = this;
        var req = $.getJSON('./js/cityes.json', function(data, status) {
            if(status === 'success') {
                _this.cityes = data.cityes;
            }
        });
    };
    Landing.prototype.autocompleteAddress = function(event) {
        if(landing.cityes) {
            var input = $(event.currentTarget);
            var helper = input.siblings('.address_helper');

            var cityName = input.val().trim();
            var result = '';
            clearTimeout(landing.cityTimeout);

            if (cityName.length > 2) {
                var re = new RegExp(cityName.toUpperCase(), 'g');
                landing.cityTimeout = setTimeout(function() {
                    helper.html('');
                    landing.cityes.forEach(function(current, index) {
                        if(current.search(re) !== -1) {
                            helper.append('<li class="js-city">' + current + '</li>');
                        }
                    });
                    if(helper.find('li').length) {
                        helper.fadeIn();
                        helper.children('.js-city').on('click', landing.selectCity);
                    } else {
                        helper.fadeOut();
                        helper.children('.js-city').off('click');
                    }
                }, 800);
            } else {
                helper.html('').fadeOut();
    ...
    Landing.prototype.selectCity = function(event) {
        var li = $(event.currentTarget);
        li.parent().siblings('.js-address').val(li.html());
        li.parent().fadeOut(100);
        $('.js-city').off('click');
    };

    Landing.prototype.checkDocClick = function(event) {
        var target = event.currentTarget;
        if(!$('.address_helper').is(target) && $('.address_helper').has(target).length === 0) {
            $('.address_helper').fadeOut(100);
            $('.js-city').off('click');
        }
    }
